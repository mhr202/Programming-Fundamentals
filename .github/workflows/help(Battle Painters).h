...
void myLine(int x1, int y1, int x2, int y2,int RB , int BB , int BG) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);

    //change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen =CreatePen(PS_SOLID,4,RGB(RB,BB,BG)); //2 is the width of the pen
...
bool isCursorKeyPressed(int& whickKey)   //whichKey passed as reference.... 
{
	...
	if (key == 1)
	{
		...
		whickKey = 2;		// 2 if n key is pressed
		return true;
	}
	...
	if (key == 1)
	{
		...
		whickKey = 4;   // 4 if m key is pressed
		return true;
	}
...
void myEllipse(float x1, float y1, float x2, float y2, int R, int G, int B, int BR, int RG, int BB)
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);
    ...
    HPEN pen =CreatePen(PS_SOLID,2,RGB(R,G,B)); 
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(BR, RG, BB));  //Fill color is black
	SelectObject(device_context,brush);
...
void myEllipse2(float &x1, float &y1, float &x2, float &y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_NULL, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}
